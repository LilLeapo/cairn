...
- **对话学习**：你问，老师答（流式）。
- **一张从探索里长出来的图**：你看不见的「观察者」边看边把概念画成**递归嵌套图**——一次看一层，双击钻进子图（放大＝扩散），面包屑爬回上层（缩小＝收拢）。
...
打开 http://localhost:5173 ，**粘你自己的 Anthropic API Key**（BYOK，存本地）。

## 技术栈

纯前端、无后端：**React + Vite + TypeScript**，`idb`（IndexedDB）存储，`@anthropic-ai/sdk` 浏览器直连（`dangerouslyAllowBrowser`）。
...
src/
  ...
  anthropic.ts   两套循环：老师(流式) + 观察者(结构化 tool call)
